...
const toSpritesheet = async (input: string[], output: string, cfg: SpritesheetConfig) => {
    ...
    for (const img of input) {
        ...
        const blockTrim = { x: 0, y: 0, w: meta.width || 0, h: meta.height || 0, used: false }
        if (cfg.trim) {
            ...
            const { info } = await sharp(flattened)
                .trim()
            ...
            buffer = await src
                .extract({ left: blockTrim.x, top: blockTrim.y, width: blockTrim.w, height: blockTrim.h })
                .toBuffer()
        } else {
        ...
        for (const block of blocks) {
            ...
            for (let y = 0; y < info.height; y++) {
                for (let x = 0; x < info.width; x++) {
                    ...
                    // edge
                    if (x >= corners.tl[0] && x <= corners.tr[0] && y < corners.tl[1]) cpPixels(corners.tl[1] * 4 * info.width + x * 4, idx)
                    if (x >= corners.tl[0] && x <= corners.tr[0] && y > corners.bl[1]) cpPixels(corners.bl[1] * 4 * info.width + x * 4, idx)
                    if (y >= corners.tl[1] && y <= corners.bl[1] && x < corners.tl[0]) cpPixels(y * 4 * info.width + corners.tl[0] * 4, idx)
                    if (y >= corners.tl[1] && y <= corners.bl[1] && x > corners.tr[0]) cpPixels(y * 4 * info.width + corners.tr[0] * 4, idx)
                }
            }

            block.buffer = await sharp(img, { raw: info })
                .png()
                .toBuffer()
        }
    }
    ...
    for (const block of blocks) {
        ...
        manifestData.frames[name] = {
            ...
            spriteSourceSize: { x: block.trim.x, y: block.trim.y, w: block.trim.w, h: block.trim.h },
