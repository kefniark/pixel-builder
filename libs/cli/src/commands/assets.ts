...
export function assetTransform() {
    ...
    const generateAssetId = (imgPath: string, url: URL, config: AssetTransformOption) => {
        ...
        const hash = createHash('sha1').update(baseURL.href).update(JSON.stringify({...config, crc: fileHash })).digest('hex')
    ...
    return {
        ...
        async load(id) {
            ...
            assetsToCopy.set(`/${resourceFolder}/${newid}.webp`, { src: srcURL.href, cache: tmpPath, id: newid })
